export class MetricsCollector {
	...
	calculate() {
		...
		const minLatency =
			this.latencies.length > 0 ? Math.min(...this.latencies) : 0

		const maxLatency =
			this.latencies.length > 0 ? Math.max(...this.latencies) : 0

		const jitter = this.latencies.length > 1 ? this.calculateJitter() : 0

		const throughputMbps =
			durationSec > 0 ? (this.bytesReceived * 8) / durationSec / 1_000_000 : 0

		const packetLossPercent =
			this.packetsSent > 0 ? (this.packetsLost / this.packetsSent) * 100 : 0

		const cpuUsageMs = this.cpuEnd
			? (this.cpuEnd.user + this.cpuEnd.system) / 1000
			: 0

		// Report current heap usage in MB (more stable than a delta which can be negative after GC)
		const memUsedMB = this.memEnd ? this.memEnd.heapUsed / 1024 / 1024 : 0
